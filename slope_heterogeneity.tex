...
If $E[x_{it}^2 \eta_i]\neq 0$ for at least one time period $t$ then $\widehat{\beta}_{OLS}$ will show a bias whereas $\widehat{\beta}_{MG}$ will not.
To encode this idea within the local mis-specification framework, we take
\begin{equation}
\sum_{t=1}^T E[x_{it}^2 \eta_i] = \frac{\tau}{\sqrt{n}}
\end{equation}
so that, for any fixed $n$ the OLS estimator a bias unless $\tau = 0$ but this bias disappears in the limit as $n \rightarrow \infty$.
