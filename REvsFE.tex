%!TEX root = main.tex
\section{Examples of the GFIC in Panel Data Models}
\label{sec:examples}
\todo[inline]{Add short paragraph introducing the examples.}
...
We see from Theorem \ref{thm:REvsFE} that $AMSE(\widehat{\beta}_{RE}) = c^2 \tau^2 + \eta^2$, $AMSE(\widehat{\beta}_{FE}) = c^2\sigma^2 + \eta^2$, and $\widehat{\tau}^2 - \sigma^2$ provides an asymptotically unbiased estimator of $\tau$.
