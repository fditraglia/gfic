%!TEX root = main.tex
\section{Notation and Asymptotic Framework}
\todo[inline]{After Minsu adds the high-level sufficient conditions, put them into a numbered assumption and update the theorems etc to refer to them}
\label{sec:asymp}
...
Let $\{Z_{ni}\colon 1\leq i \leq n, n =1, 2, \hdots\}$ be a triangular array of random vectors defined on a probability space $(\Upsilon, \mathcal{F},P)$ satisfying
	\begin{enumerate}[(a)]
		\item $E[g(Z_{ni}, \gamma_n,\theta_0)]=0$
		\item $E[h\left( Z_{ni}, \gamma_n, \theta_0\right)] = \tau_n$
		\item $\{f(Z_{ni}, \gamma_n, \theta_0)\colon 1\leq i \leq n, n = 1, 2, \hdots \}$ is uniformly integrable, and 
		\item $Z_{ni}\overset{d}{\rightarrow} Z_i$, where the $Z_i$ are identically distributed.
	\end{enumerate}
...
To simplify the proofs we make the following further assumption concerning the triangular array from Assumption \ref{assump:local}, although it is not strictly necessary.
\begin{assump}
\label{assump:add}
$\{Z_{ni}\colon 1\leq i \leq n, n =1, 2, \hdots\}$ is $iid$ over $i$ for fixed $n$.
\end{assump}
Note that, by Assumptions \ref{assump:local}--\ref{assump:add}, the limiting random variable $Z_i$ satisfies the population moment condition $E[f\left(Z_i,\gamma_0, \theta_0\right)]=0$. 
...
Because Assumption \ref{assump:local} ensures that they are correctly specified in the limit, \emph{all} candidate specifications $(b,c)\in \mathcal{BC}$ provide consistent estimators of $\theta_0$ under standard, high level regularity conditions.
\todo[inline]{Add high-level sufficient conditions! These should be an explicit assumption.}
...
\label{thm:asymp}
Under Assumptions \ref{assump:local}--\ref{assump:add} and standard regularity conditions,
		\begin{equation}
...
where $\beta_0^{(b)'} = (\theta_0, \gamma_0^{(b)})$,
	\begin{equation}
		K(b,c) = \left[F(b,c)'W_c F(b,c)\right]^{-1} F(b,c)' W_c 
	\end{equation}
and
	\begin{equation}
