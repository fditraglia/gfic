...
Next, define $L^k\Delta y_i^{+} = [\Delta y_{i1}, \Delta y_{i2}, \cdots, \Delta y_{iT-k}]'$ where $L^k$ denotes the element-wise application of the $k$\textsuperscript{th} order lag operator.
...
Finally, stacking over individuals in the usual way, let  
\begin{equation}
 \Delta \mathbf{y}^{+} = \left[
 \begin{array}{c}
   \Delta \mathbf{y}_1^{+}\\
 ...
 \right]
\end{equation}
where $\ell$ is either $r$ or $k$ and similarly define $Z'(\ell,\cdot) = [Z'_1(\ell,\cdot) \cdots Z'_n(\ell,\cdot)]$ where $(\cdot)$ is $\text{P}$ or $\text{S}$ depending on which instrument set is in use.
Using this notation, the true model is
\begin{align}
...
\begin{equation}
  \widehat{Q}= \left[ \left( \frac{W'Z}{n} \right)\left( \frac{Z'Z}{n} \right)^{-1}\left( \frac{Z'W}{n} \right)^{-1} \right]\left( \frac{W'Z}{n} \right)\left( \frac{Z'Z}{n} \right)^{-1}
\end{equation}
we can define our estimators as follows
\begin{align}
  \widehat{\beta}(k,\cdot) &= \widehat{Q}(k,\cdot)\left[ \frac{Z'(r,\text{S})\Delta \mathbf{y}}{n} \right]& 
  \widehat{\beta}(r,\cdot) &= \widehat{Q}(r,\cdot)\left[ \frac{Z'(r,\text{S})\Delta \mathbf{y}^{+}}{n} \right]
\end{align}
where $(\cdot)$ is either $\text{P}$ or $\text{S}$ depending on which instrument set is used.

\begin{thm}
  Under the model, local mis-specification, and standard regularity conditions
  \begin{align*}
  ...
  \end{align*}
  for the estimators based on the true lag length $k$ versus
  \begin{align*}
    \sqrt{n}\left[ \widehat{\beta}(r,\text{P})- \beta_r \right] &\rightarrow^d Q(r,\text{P}) \left[\boldsymbol{\iota}_{T-k} \otimes \delta \psi_{\text{P}} + \mbox{N}\left(\mathbf{0}, \mathcal{V}(r,\text{P}) \right) \right]\\
    \sqrt{n}\left[ \widehat{\beta}(r,\text{S})- \beta_r\right] &\rightarrow^d Q(r,\text{S}) \left[\boldsymbol{\iota}_{T-k} \otimes 
    \left( \delta + \left[
  \begin{array}{c}
    \psi_{\text{P}} \\ \psi_{\text{S}}
\end{array}
\right] + \left[
\begin{array}{c}
  \mathbf{0}_{k} \\ \tau
...
\right]\right) + \mbox{N}\left( \mathbf{0}, \mathcal{V}\left(r,\text{S}\right) \right)\right]
  \end{align*}
  for the estimators based in the incorrect lag length $r = k-1$, where $\mathcal{V}(k,\cdot) = \mbox{Var}\left[ Z_i(k,\cdot) \Delta \mathbf{v}_i  \right]$ and $\mathcal{V}(r,\cdot) = \mbox{Var}\left[ Z_i(r,\cdot) \Delta \mathbf{v}^{+}_i  \right]$, $Q(\ell,\cdot)$ is the probability limit of $\widehat{Q}(\ell,\cdot)$, $\psi_{\text{P}} = E[\textbf{z}_{it}(k-1,\text{P}) \Delta y_{it -k}]$, and $\psi_{\text{S}} = E[x_{it} \Delta y_{it-k}]$.
\end{thm}
...
Expanding, we have 
\begin{align}
  \sqrt{n}\left[\widehat{\beta}(k,\cdot) - \beta\right] &=(\mathbf{0}', \delta, 0)' + \widehat{Q}(k,\cdot)\left[ \frac{Z'(k,\cdot)\Delta \mathbf{v}}{\sqrt{n}} \right]\
\end{align}
for the specifications that include all $k$ lags versus
\begin{align}
  \sqrt{n}\left[\widehat{\beta}(r,\cdot) - \beta_{r}\right] &=  \widehat{Q}(r,\cdot)\left\{ \delta \left[\frac{Z'(r,\cdot) L^{k}\Delta \mathbf{y}^+}{n}\right] + \left[\frac{Z'(r,\cdot)\Delta \mathbf{v}^{+}}{\sqrt{n}}\right] \right\}
\end{align}
for the specifications that include only $r = k-1$ lags.

\begin{align*}
  \frac{Z'(r,\cdot) L^k \Delta \mathbf{y}^+}{n} = \frac{1}{n}\sum_{i=1}^n Z'(r,\cdot) L^k \Delta \mathbf{y}^{+}  
\end{align*}
so we need $E[\mathbf{z}_{it}(r,\cdot) \Delta y_{it - r - 1}]$ which we'll call $\psi_{\text{S}}$ and  $(\psi_{\text{P}}, \psi_{\text{S}})$.
\end{proof}
