...
\todo[inline]{Explain about the two instrument sets, borrowing text from below. The point here is to unify as much of the discussion as possible for the two special cases we'll explore below. Explain how many time periods a model with lag length $\ell$ uses. Then say that we're going to specialize to two simple cases below. Is there a way to be more general in the expressions that I write down? That would be ideal. Then we could always specialize to the case that appears in the simulation.}

\todo[inline]{Explain what will be local to zero!}
