...
Given these instrument sets, we have $(\ell + 1)\times T$ moment conditions if $x$ is assumed to be predetermined versus $(\ell + 2)\times T$ if it is assumed to be strictly exogenous, corresponding to the instrument matrices
\begin{align}
...
Expanding, we have 
\begin{align}
  \sqrt{n}\left[\widehat{\beta}(k,\cdot) - \beta\right] &=(\mathbf{0}', \delta, 0)' + \widehat{Q}(k,\cdot)\left[ \frac{Z'(r,\cdot)\Delta \mathbf{v}}{\sqrt{n}} \right]\
\end{align}
for the specifications that include all $k$ lags versus
\begin{align}
  \widehat{\beta}(r,\cdot) &=  \widehat{Q}(r,\cdot)\left\{ \delta \left[\frac{Z'(r,\cdot) L^{k-1}\Delta \mathbf{y}^+}{n}\right] + \left[\frac{Z'(r,\cdot)\Delta \mathbf{v}^{+}}{\sqrt{n}}\right] \right\}
\end{align}
for the specifications that include $r = k-1$ lags.
