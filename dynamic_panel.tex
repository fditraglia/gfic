...
The sample analogue $\widehat{Q}(k,\text{P})$ is a consistent for $\widehat{Q}(k,\text{P})$, as mentioned above, and
\begin{equation}
...
Expressed as a function of the underlying model parameters, the limiting value of $\mu_{LR}$ is
\[
  \mu_{LR}^{0} = \varphi(\beta) =  \varphi(\boldsymbol{\gamma}_r, 0, \theta) = \frac{\theta}{1 - \boldsymbol{\iota}_{r}' \boldsymbol{\gamma}_r}
\]
while the true value, which changes with sample size, is
\[
  \mu_{LR}^{n} = \varphi(\beta_n) =  \varphi(\boldsymbol{\gamma}_r, \delta/\sqrt{n}, \theta) = \frac{\theta}{1 - \boldsymbol{\iota}_{r}' \boldsymbol{\gamma}_r - \delta/\sqrt{n}}.
\]

\begin{thm}[Asymptotic Distribution of $\mu_{LR}$ Estimators]

\end{thm}

\todo[inline]{Fix from here down. Should probably encapsulate the result within a Corollary.}
