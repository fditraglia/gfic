...
\right].
\end{align*}
Note that $X_i^{+}(r)$ contains one more row than $X_i(k)$ but $X_i(k)$ contains one more column that $X_i^{+}(r)$: removing the $k$\textsuperscript{th} lag from the model by setting $\ell = r = k-1$ allows us to use an additional time period in estimation and reduces the number of regressors by one. 
...
\begin{align}
  \Delta \mathbf{y} &= X(k) + \Delta \mathbf{v} &  \Delta \mathbf{y}^{+} &= X(k)^{+} + \Delta \mathbf{v}^+.
\end{align}
Now, using the shorthand
\begin{equation}
  \widehat{Q}= \left[ \left( \frac{X'Z}{n} \right)\left( \frac{Z'Z}{n} \right)^{-1}\left( \frac{Z'X}{n} \right)^{-1} \right]\left( \frac{X'Z}{n} \right)\left( \frac{Z'Z}{n} \right)^{-1}
\end{equation}
we can define our four estimators as follows:
\begin{align}
  \widehat{\beta}(k,\text{P}) &= \widehat{Q}(k,\text{P})\left[ \frac{Z(r,\text{S})\Delta y}{n} \right]& 
  \widehat{\beta}(k,\text{S}) &= \widehat{Q}(k,\text{S})\left[ \frac{Z(r,\text{S})\Delta y}{n} \right]\\ 
  \widehat{\beta}(r,\text{P}) &= \widehat{Q}(r,\text{P})\left[ \frac{Z(r,\text{S})\Delta y^{+}}{n} \right]& 
  \widehat{\beta}(r,\text{S}) &=  \widehat{Q}(r,\text{S})\left[ \frac{Z(r,\text{S})\Delta y^{+}}{n} \right].
\end{align}
Expanding,
\begin{align}
  \sqrt{n}\left[\widehat{\beta}(k,\text{P}) - \beta\right] &= \widehat{Q}(k,\text{P})\left[ \frac{Z(r,\text{S})\Delta y^{+}}{n} \right]\\ 
  \sqrt{n}\left[\widehat{\beta}(k,\text{S}) - \beta\right] &= \widehat{Q}(k,\text{S})\left[ \frac{Z(r,\text{S})\Delta y^{+}}{n} \right]\\ 
  \widehat{\beta}(r,\text{P}) &= \widehat{Q}(r,\text{P})\left[ \frac{Z(r,\text{S})\Delta y^{+}}{n} \right]\\
  \widehat{\beta}(r,\text{S}) &=  \widehat{Q}(r,\text{S})\left[ \frac{Z(r,\text{S})\Delta y^{+}}{n} \right]
\end{align}
