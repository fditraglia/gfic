...
\todo[inline]{Need to fix things a bit from here down. Should mention that the following result is slightly different from the general theorem given above since we impose the structure of the panel dataset to get a pooled estimator of $\tau$ which brings in the time averages.}

\begin{thm}[Joint Limit Distribution of $\widehat{\delta}$ and $\widehat{\tau}$]
  Under the conditions of Theorem \ref{thm:limitDpanel},
  \[\left[\begin{array}{c} \widehat{\delta} \\ \widehat{\tau}  \end{array} \right] \overset{d}{\rightarrow} \left[\begin{array}{c} \delta \\ \tau  \end{array} \right]  + \Psi N(0, \Pi\,\mathcal{V}(k,\text{S})\,\Pi')\]
...
\right].
  ...
  \right] \widehat{Q}(k,\text{P}) n^{-1/2} Z'(k,\text{P})\Delta \mathbf{v}.
  \]
  Now, by a law of large numbers $\widehat{Q}(k,\text{P}) \rightarrow_p Q(k,\text{P})$ and $n^{-1}X'W(k) \rightarrow_p \boldsymbol{\xi}' \otimes \boldsymbol{\iota}_{T-k-1}$ where 
  \[
    \boldsymbol{\xi}' = E\left\{ x_{it} \left[
    \begin{array}{ccccc}
      L \Delta y_{it} & L^2 \Delta y_{it} & \cdots & L^k \Delta y_{it} & \Delta x_{it}
  \end{array}
\right]\right\} 
  \]
  so it follows that
...
\right] + o_p(1)
  \]
  where $\mathbf{e}_k = (\mathbf{0}_{k-1}', 1, 0)'$ is the $k$\textsuperscript{th} standard basis vector in $(k+1)$-dimensional space,  and 
\[
  \Psi = \left[
  \begin{array}{cc}
...
\right].
\]
The result follows, after some algebra, by applying the Lindeberg-Feller central limit theorem jointly to $n^{-1/2}Z'(k,\text{P})\Delta\mathbf{v}$ and $n^{-1/2}X'\Delta \mathbf{v}$.
\end{proof}
