...
\[
  \mu_{LR}^{0} = \varphi(\beta) =  \varphi(\theta, \boldsymbol{\gamma}_r, 0) = \frac{\theta}{1 - \boldsymbol{\iota}_{r}' \boldsymbol{\gamma}_r}
\]
while the true value, which changes with sample size, is
\[
  \mu_{LR}^{n} = \varphi(\beta_n) =  \varphi(\theta, \boldsymbol{\gamma}_r, \delta/\sqrt{n}) = \frac{\theta}{1 - \boldsymbol{\iota}_{r}' \boldsymbol{\gamma}_r - \delta/\sqrt{n}}.
\]

\begin{cor}[Asymptotic Distribution of $\mu_{LR}$ Estimators]
