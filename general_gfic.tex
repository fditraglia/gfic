...
Denote this target parameter by $\mu = \varphi(\theta, \gamma)$, where  $\varphi$ is a real-valued, almost surely continuous function of the underlying model parameters $\theta$ and $\gamma$.
Let $\mu_n = \varphi(\theta_0,\gamma_n)$ and define $\mu_0$ and $\widehat{\mu}(b,c)$  analogously.
...
where $\varphi_0 = \varphi(\theta_0, \gamma_0)$.
\end{cor}
...
First, like $\tau$, it shifts the limit distribution of $\sqrt{n}f_n(\theta_0, \gamma_0)$ away from zero, thereby influencing the asymptotic behavior of $\sqrt{n}\left(\widehat{\mu}(b,c) - \mu_0 \right)$. 
Second, unless the derivative of $\varphi$ with respect to $\gamma$ is zero at $(\theta_0,\gamma_0)$, $\delta$ induces a second source of bias when $\widehat{\mu}(b,c)$ is recentered around $\mu_n$. 
...
where
\begin{equation}
	M(b,c) = \Xi_b'K(b,c) \Xi_c \left[\begin{array}{cc} -G_\gamma & 0 \\ -H_\gamma & I \end{array} \right] +
  \left[\begin{array}{ll} 
  0_{p\times r} & 0_{s\times q} \\
    I_r & 0_{r\times q}  
\end{array} 
  \right]
\end{equation}
...
where 
$K_v = \left[G'W_{gg}G\right]^{-1}G'W_{gg} = \left(K_{v}^{\theta'}, K_{v}^{\gamma'}\right)'$, so $\widehat{\delta}$ is an asymptotically unbiased estimator of $\delta$.
\end{cor} 
...
\]
	where $K_v = \left[G'W_{gg}G\right]^{-1}G'W_{gg}$  is partitioned according to $K_v' = (K_v^{\theta'}, K_v^{\gamma'})$.
\end{thm}
